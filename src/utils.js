    ...
    if (isArrayLike(val)) {
        ...
        for (key in val) {
            if (val.hasOwnProperty(key)) {
                target[key] = deep ? clone(val[key], deep) : val[key];
            }
        }
    }
...
export const isPCBroswer = () => {
    ...
    let t = e.match(/ipad/i) === 'ipad';
    let i = e.match(/iphone/i) === 'iphone';
    let r = e.match(/midp/i) === 'midp';
    let n = e.match(/rv:1.2.3.4/i) === 'rv:1.2.3.4';
    let a = e.match(/ucweb/i) === 'ucweb';
    let o = e.match(/android/i) === 'android';
    let s = e.match(/windows ce/i) === 'windows ce';
    let l = e.match(/windows mobile/i) === 'windows mobile';
 ...
 */
export const fill = (value, start, end) => {
    let length = this.length;
    start = String.parseInt(start);
    if (isNaN(start)) {
        start = 0;
    } else if (start < 0) {
        start = -start > length ? 0 : length + start;
    }
    end = String.parseInt(end);
 ...
 */
export const includes = (value, start) => {
    let length = this.length;
    start = String.parseInt(start);
    if (isNaN(start)) {
        start = 0;
